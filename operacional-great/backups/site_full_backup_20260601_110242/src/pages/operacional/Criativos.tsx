...
import { Plus, CheckCircle2, Upload, FileIcon, User, Calendar, Clock, Trash2, Loader2, ChevronsUpDown, Check, Download, BarChart3, Filter, PencilLine } from 'lucide-react';
...
const AD_CREATIVE_UPLOAD_TIMEOUT_MS = 120_000;
const AD_CREATIVE_SAVE_TIMEOUT_MS = 120_000;
const CREATIVE_RESPONSIBLES = ['Amanda', 'Matheus'] as const;
...
export default function Criativos() {
  ...
  const creativeResponsibles = useMemo(() => [...CREATIVE_RESPONSIBLES], []);
  ...
  const { data: adCreatives = [], isLoading } = useQuery({
    ...
    placeholderData: () => mergeAdCreativeCollections(getSeedAdCreatives(), getOfflineAdCreatives()),
    staleTime: 30_000,
  ...
  const upsertActivityTracking = async (clientId: string, designerName: string) => {
    ...
    const { data: existing } = await withTimeout(
      supabase
        ...
        .maybeSingle(),
      10_000,
      'Leitura de rastreamento de atividades',
    );
    ...
    if (existing) {
      await withTimeout(
        supabase
          ...
          .eq('id', existing.id),
        10_000,
        'Atualização de rastreamento de atividades',
      );
    } else {
      await withTimeout(
        supabase
        ...
        10_000,
        'Criação de rastreamento de atividades',
      );
    }
  };
  ...
  const openNewCreativeDialog = () => {
    resetCreativeForm();
    setIsAddOpen(true);
  };

  const closeCreativeDialog = () => {
    setIsAddOpen(false);
    resetCreativeForm();
  };

  const openEditCreative = (ad: AdCreative) => {
    setEditingAd(ad);
    setNewClientName(ad.client_name);
    setNewClientId(ad.client_id);
    setManualClientMode(!ad.client_id);
    setSelectedFiles([]);
    setPreviewUrls([]);
    setResponsavelArte(ad.created_by_name || '');
    setIsAddOpen(true);
  };
  ...
  const addCreative = useMutation({
    mutationFn: async () => {
      ...
      const uploadFiles = async (files: File[]) => Promise.all(
        files.map(async (file) => {
          const fileExt = file.name.split('.').pop();
          const filePath = `${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`;
          const { error: uploadError } = await withTimeout(
            supabase.storage.from('ad-creatives').upload(filePath, file),
            AD_CREATIVE_UPLOAD_TIMEOUT_MS,
            'Upload do anúncio',
          );
      ...
      try {
        ...
        const { data, error } = await withTimeout(
          supabase.from('ad_creatives').insert(payload).select().single(),
          10_000,
          'Criação do anúncio',
        );
        if (error) throw error;
  ...
  const saveCreativeOnline = useMutation({
    mutationFn: async () => {
      if (!canManageCreatives) throw new Error('Voce nao tem permissao para salvar criativos');
      if (!user || !newClientName.trim() || !responsavelArte) throw new Error('Dados obrigatorios faltando');
      if (!editingAd && selectedFiles.length === 0) throw new Error('Selecione ao menos um arquivo');
      ...
      if (selectedFiles.length > 0) {
        uploadedUrls.push(...await uploadFiles(selectedFiles));
      }

      const currentUrls = editingAd?.image_urls && editingAd.image_urls.length > 0
        ? editingAd.image_urls
        : (editingAd?.image_url ? [editingAd.image_url] : []);
      const finalUrls = uploadedUrls.length > 0 ? uploadedUrls : currentUrls;
      const now = new Date().toISOString();

      const payload = {
        id: editingAd?.id ?? crypto.randomUUID(),
        client_name: newClientName.trim(),
        client_id: newClientId,
        image_url: finalUrls[0] ?? '',
        image_urls: finalUrls,
        created_by_user_id: user.id,
        created_by_name: responsavelArte,
        status: (editingAd?.status ?? 'PARA_SUBIR') as 'PARA_SUBIR' | 'ATIVO',
        completed_by_user_id: editingAd?.completed_by_user_id ?? null,
        completed_by_name: editingAd?.completed_by_name ?? null,
        completed_at: editingAd?.completed_at ?? null,
        created_at: editingAd?.created_at ?? now,
        updated_at: now,
      };
      ...
      const { data, error } = await withTimeout(
        supabase
          ...
          .select()
          .single(),
        AD_CREATIVE_SAVE_TIMEOUT_MS,
        'Salvamento do anúncio',
      );
      if (error) throw error;

      if (!editingAd && newClientId && responsavelArte) {
        try {
          await upsertActivityTracking(newClientId, responsavelArte);
        } catch (e) {
          console.error('Error updating activity tracking:', e);
    ...
    onSuccess: () => {
      toast.success(editingAd ? 'Anuncio atualizado e sincronizado!' : 'Anuncio adicionado e sincronizado!');
      closeCreativeDialog();
      queryClient.invalidateQueries({ queryKey: ['ad-creatives'] });
      queryClient.invalidateQueries({ queryKey: ['client-activity-tracking'] });
      queryClient.invalidateQueries({ queryKey: ['designers-totals'] });
  ...
  const completeCreativeOnline = useMutation({
    mutationFn: async ({ id, gestorName }: { id: string; gestorName: string }) => {
    ...
    onSuccess: () => {
      toast.success('Anuncio marcado como ativo e sincronizado!');
      setActivateAd(null);
      setActivateGestor('');
      queryClient.invalidateQueries({ queryKey: ['ad-creatives'] });
    },
  ...
  const deleteCreativeOnline = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('ad_creatives').delete().eq('id', id);
      if (error) throw error;
      return { ok: true };
    ...
    onSuccess: () => {
      toast.success('Anuncio removido e sincronizado!');
      queryClient.invalidateQueries({ queryKey: ['ad-creatives'] });
    },
    onError: (err: any) => toast.error('Erro: ' + err.message),
  });
  ...
  const handleConfirmActivate = () => {
    ...
    completeCreativeOnline.mutate({ id: activateAd.id, gestorName: activateGestor });
  };
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          {canManageCreatives ? (
            <Button onClick={openNewCreativeDialog} className="gap-2">
            ...
            {isLoading ? (
              ...
              paraSubir.map((ad) => (
                <AdCard
                  ...
                  onDelete={() => deleteCreativeOnline.mutate(ad.id)}
                  onEdit={canManageCreatives ? () => openEditCreative(ad) : undefined}
      ...
      {/* Add Dialog */}
      <Dialog open={isAddOpen} onOpenChange={(open) => (open ? setIsAddOpen(true) : closeCreativeDialog())}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingAd ? 'Editar Anúncio' : 'Novo Anúncio para Subir'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">
                {editingAd ? 'Arquivos do Anúncio' : 'Arquivos do Anúncio *'}
              </label>
              {editingAd && selectedFiles.length === 0 && (
                <p className="mb-2 text-xs text-muted-foreground">
                  Nenhum novo arquivo selecionado. Os arquivos atuais serao mantidos.
                </p>
              )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={closeCreativeDialog}>Cancelar</Button>
            <Button
              onClick={() => saveCreativeOnline.mutate()}
              disabled={!canManageCreatives || !newClientName.trim() || !responsavelArte || (!editingAd && selectedFiles.length === 0) || isUploading}
            >
              {isUploading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Plus className="h-4 w-4 mr-2" />}
              {editingAd ? 'Salvar alterações' : 'Adicionar'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!activateGestor || completeCreativeOnline.isPending}
              className="gap-2"
            >
              {completeCreativeOnline.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
              Ativar
            </Button>
          </DialogFooter>
        </DialogContent>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter className="pt-2">
          ...
          {canEdit && onEdit && (
            <Button onClick={() => onEdit(ad)} className="gap-2">
...
function AdCard({
  ...
  const canEdit = isAdmin || ad.created_by_user_id === userId;
  const urls = ad.image_urls && ad.image_urls.length > 0 ? ad.image_urls : (ad.image_url ? [ad.image_url] : []);
    ...
    >
        ...
        <div className="relative">
          ...
          {canEdit && onEdit && (
            <button
              ...
              className="absolute top-2 right-11 bg-sky-500 hover:bg-sky-600 text-white rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
              title="Editar"
            >
      ...
      {urls.length === 0 && (
        <div className="relative px-3 pt-3 flex justify-end gap-1">
          ...
          {canEdit && onEdit && (
            <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="bg-sky-500 hover:bg-sky-600 text-white rounded-full p-1.5 transition-colors shadow-sm" title="Editar">
              <PencilLine className="h-3.5 w-3.5" />
            </button>
          )}
          {canDelete && (
