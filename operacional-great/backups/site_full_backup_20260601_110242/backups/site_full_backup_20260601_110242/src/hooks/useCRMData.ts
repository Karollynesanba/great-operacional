...
let operationalClientsRealtimeSubscribed = false;
...
export function clearOperationalClientCache() {
  safeSetItem(OPERATIONAL_CLIENTS_CACHE_KEY, JSON.stringify([]));
}
...
export function useOperationalClients() {
  ...
  useEffect(() => {
    if (operationalClientsRealtimeSubscribed) return;

    operationalClientsRealtimeSubscribed = true;
    supabase
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const serverClients = (data || []) as OperationalClient[];
    ...
    placeholderData: () => readOperationalClientCache(),
    staleTime: 30_000,
...
export function useCreateOperationalClient() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const activatedAtIso = data.activated_at ? new Date(data.activated_at).toISOString() : null;
