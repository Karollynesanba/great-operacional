...
const CLIENT_FILE_UPLOAD_TIMEOUT_MS = 120_000;
const CLIENT_FILE_SAVE_TIMEOUT_MS = 120_000;
...
export default function ClienteDetalhes() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!client) {
      toast.error('Cliente ainda não carregou. Tente novamente em alguns segundos.');
      return;
    }
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const { error: uploadError } = await withTimeout(
          supabase.storage.from('client-files').upload(filePath, file),
          CLIENT_FILE_UPLOAD_TIMEOUT_MS,
          'Upload do arquivo do cliente',
        );
        if (uploadError) throw uploadError;
        const { data: urlData } = supabase.storage.from('client-files').getPublicUrl(filePath);
        const { error: fileInsertError } = await withTimeout(
          ...
          CLIENT_FILE_SAVE_TIMEOUT_MS,
          'Salvamento do arquivo do cliente',
        );
      ...
      console.error('Erro ao enviar arquivo do cliente:', error);
