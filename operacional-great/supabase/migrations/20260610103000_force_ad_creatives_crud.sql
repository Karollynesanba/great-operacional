-- Force CRUD access for ad creatives and keep uploads visible to every session.
...
ALTER TABLE IF EXISTS public.ad_creatives REPLICA IDENTITY FULL;
...
-- Keep the updated_at column synchronized on every edit.
...
-- ---------------------------------------------------------------------------
-- Realtime so all sessions receive insert/update/delete immediately
-- ---------------------------------------------------------------------------
DO $$
BEGIN
  IF NOT EXISTS (
