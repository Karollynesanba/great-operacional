-- Manual repair script for the shared login/auth schema.
-- Paste this entire file into the Supabase SQL editor and run it once.
-- Safe to re-run: it uses IF NOT EXISTS / DROP IF EXISTS where needed.
...
SELECT
  ...
  COALESCE(
    LOWER(COALESCE(u.raw_app_meta_data ->> 'role', u.raw_user_meta_data ->> 'role', '')) = 'admin',
    false
  ),
