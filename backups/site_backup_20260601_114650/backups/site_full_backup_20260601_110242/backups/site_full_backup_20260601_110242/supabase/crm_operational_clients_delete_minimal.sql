-- Minimal delete fix for CRM operational clients.
-- Run this if the front still shows "Erro ao excluir cliente".
...
CREATE OR REPLACE FUNCTION public.operational_client_delete_cleanup()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP TRIGGER IF EXISTS operational_clients_delete_cleanup ON public.operational_clients;
CREATE TRIGGER operational_clients_delete_cleanup
BEFORE DELETE ON public.operational_clients
FOR EACH ROW
EXECUTE FUNCTION public.operational_client_delete_cleanup();
