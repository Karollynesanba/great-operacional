...
export function useProjectsData() {
  ...
  const { data: projects = [], isLoading: isLoadingProjects, error: projectsError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const projectRows = (data || []) as (Project & { owner_user_ids?: string[] | null })[];
      const ownerIds = Array.from(
        new Set(
          projectRows.flatMap((project) =>
            Array.isArray(project.owner_user_ids) ? project.owner_user_ids.filter(Boolean) : [],
          ),
        ),
      );

      const ownersById = new Map<string, { id: string; full_name: string; avatar_url: string | null }>();
      ...
      return projectRows.map((project) => ({
        ...project,
        owners: Array.isArray(project.owner_user_ids)
          ? project.owner_user_ids
              .map((ownerId) => ownersById.get(ownerId))
              .filter((owner): owner is { id: string; full_name: string; avatar_url: string | null } => Boolean(owner))
          : [],
      })) as Project[];
    },
    placeholderData: () => queryClient.getQueryData<Project[]>(['projects']) ?? [],
    staleTime: 30_000,
