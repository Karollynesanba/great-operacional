...
function getCachedMeetings() {
  return filterDeletedMeetings(filterLegacyMeetings(readOfflineCollection<Meeting>('meetings')));
}
...
export function useWorkItems() {
  return useQuery({
    queryKey: ['work-items'],
    queryFn: async () => {
      const cache = getCachedWorkItems();
      try {
    ...
    placeholderData: () => getCachedWorkItems(),
    staleTime: 30_000,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (error) throw error;
        const nextItems = dedupeWorkItems(filterLegacyWorkItems((data as WorkItem[]) || []));
        writeOfflineCollection('work-items', nextItems);
        return nextItems
          .filter((item) => !COMPLETED_WORK_ITEM_STATUSES.includes(item.status as (typeof COMPLETED_WORK_ITEM_STATUSES)[number]))
          .sort((a, b) => {
            const aHasDueDate = !!a.due_date;
        ...
        return getCachedUpcomingTasks(limit);
      }
    },
    placeholderData: () => getCachedUpcomingTasks(limit),
    staleTime: 30_000,
    refetchOnWindowFocus: false,
  });
}
...
export function useMeetings(options: QueryOptions = {}) {
  return useQuery({
    ...
    placeholderData: () => getCachedMeetings(),
    staleTime: 30_000,
    refetchOnWindowFocus: false,
  });
}
