...
function dedupeUsersByDisplayName(users: { id: string; full_name: string; email?: string | null }[]) {
...
async function persistMyDayAssignments(params: {
  ...
  try {
    const { data, error } = await withTimeout(
      supabase.functions.invoke('sync-my-day-items', {
        body: {
          items: payload,
  ...
  try {
    const { data, error } = await withTimeout(
      supabase
        ...
        .eq('item_date', today),
      10_000,
      'Carregamento de exclusões do Meu Dia',
    );
...
export default function MeuDia() {
  ...
  const canViewOtherUsers = !!user;
  const canEditOtherUsersDay = isAdmin;
  ...
  const uniqueVisibleUsers = dedupeUsersByDisplayName(visibleUsers);
  const dayFilterUsers = uniqueVisibleUsers;
  const assignableUsers = (isAdmin ? visibleUsers : allUsers).filter((member) => !shouldHideFromAssignmentList(member));
  const uniqueAssignableUsers = dedupeUsersByDisplayName(assignableUsers);
  ...
  useEffect(() => {
    ...
    const loadProfiles = async () => {
      try {
        ...
        const mappedUsers = dedupeUsersByDisplayName((data || [])
        ...
        const fallbackUsers = dedupeUsersByDisplayName(users
        ...
        if (isMounted) {
          setAllUsers(dedupeUsersByDisplayName([...mappedUsers, ...fallbackUsers]));
        }
      } catch (error) {
        ...
        setAllUsers(
          dedupeUsersByDisplayName(users
            .filter((member) => member.active)
            .map((member) => ({
              id: member.id,
              full_name: member.name,
  ...
  // Fetch panorama data (admin sees all teams; other users stay on the day view)
  useEffect(() => {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const permanentSeedPromise = withTimeout(
        ensurePermanentActivities(targetUserId),
        10_000,
        'Garantia de tarefas permanentes',
      ).catch((permanentError) => {
        console.warn('Permanent activities could not be ensured for this user:', permanentError);
        return 0;
      });

      const exclusionsPromise = loadMyDayExclusions(targetUserId, today);

      // Carry over: move uncompleted non-permanent tasks from previous days to today
      try {
        const { data: pendingOld, error: carryError } = await withTimeout(
          supabase
            .from('my_day_items')
            .select('id')
            .eq('user_id', targetUserId)
          ...
          10_000,
          'Transferência de tarefas pendentes',
        );
        ...
        if (pendingOld && pendingOld.length > 0) {
          ...
          const { error: carryUpdateError } = await withTimeout(
            supabase
              .from('my_day_items')
              .update({ date: today })
              .in('id', ids),
            10_000,
            'Atualização de tarefas pendentes',
          );
      ...
      try {
        const { error: cleanupError } = await withTimeout(
          supabase
            ...
            .neq('status', 'CONCLUIDO'),
          10_000,
          'Limpeza de tarefas permanentes antigas',
        );
      ...
      // Fetch today's tasks
      try {
        const [{ data: fetchedData, error }, exclusions] = await Promise.all([
          withTimeout(
            supabase
              .from('my_day_items')
              .select('*')
        ...
        void permanentSeedPromise.then((insertedCount) => {
          if (insertedCount > 0 && fetchGenerationRef.current === fetchGeneration) {
            void fetchItems();
          }
        });
      } catch (itemsError) {
      ...
      if (workItemIds.length > 0) {
        try {
          const { data: workItems, error: workItemsError } = await withTimeout(
            supabase
              .from('work_items')
              .select('id, reporter_user_id, assignee_user_id, tags')
              .in('id', workItemIds),
            10_000,
            'Carregamento de metadados das tarefas',
          );
      ...
      if (reporterUserIds.size > 0) {
        try {
          const { data: reporterProfiles, error: reporterProfilesError } = await withTimeout(
            supabase
              .from('profiles')
              .select('id, full_name, email')
              .in('id', Array.from(reporterUserIds)),
            10_000,
            'Carregamento de perfis referentes',
          );
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      if (newItemSource === 'MANUAL') {
        try {
          const { data: workItem, error: workItemError } = await withTimeout(
            supabase
              .from('work_items')
              .insert({
      ...
      const myDaySource = (newItemSource === 'MANUAL' ? 'WORK_ITEM' : newItemSource) as MyDayItem['source'];
      ...
      try {
        ...
        const offlineItems: MyDayItem[] = myDayUserIds.map((targetUserId) => ({
          id: crypto.randomUUID(),
          user_id: targetUserId,
          title: newItemTitle.trim(),
          status: 'PENDENTE' as const,
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          {isAdmin && (
            ...
            {/* Day filter for all authenticated users */}
          {canViewOtherUsers && (
      ...
      {isLoading && visibleItems.length === 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-border bg-card/80 px-4 py-3 text-sm text-muted-foreground shadow-sm">
          <Loader2 className="h-4 w-4 animate-spin text-primary" />
          Carregando tarefas do dia...
        </div>
      )}

      {/* Quick Add - Only when viewing own day or when an admin is viewing another user */}
      {(isViewingOwnDay || canEditOtherUsersDay) && (
      ...
      {!isViewingOwnDay && (
        <div className="rounded-lg border border-info/30 bg-info/5 p-4 flex items-center gap-3">
          ...
          <p className="text-body text-info">
            ...
            {canEditOtherUsersDay ? ' — você pode adicionar e alterar tarefas' : ' (somente leitura)'}
          </p>
        </div>
      )}
          ...
          {visibleItems.length === 0 ? (
            ...
            [...pendingItems, ...completedItems].map((item) => (
              <ItemCard 
                ...
                readOnly={!isViewingOwnDay && !canEditOtherUsersDay}
              />
            ))
          )}
        </div>
